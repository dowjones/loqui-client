...
function BatchQueue(opts,client,batch,putDb,batchDb){
  ...
  this.quota = rate;
  this.throttling = typeof rate !== 'undefined' && typeof window !== 'undefined';
...
BatchQueue.prototype.queue = function(obj,method){
  ...
  if (method === 'error') {
    ...
    write = true
      ...
      if (this.batchIndexes[key] && obj.value.method === 'counter') {
        this.batch[batchIndexes[key]].value.counter += n;
      } else {
        this.batchIndexes[key] = this.batch.length;
      }
...
BachQueue.prototype.getOp = function(obj,method) {
